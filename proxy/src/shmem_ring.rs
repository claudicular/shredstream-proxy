...
impl ShmemRingProducer {
    ...
    pub fn create_with_size(path: &Path, data_region_size: usize) -> io::Result<Self> {
        ...
        // Keep the file open (mmap keeps it alive even if fd is closed,
        // but we let the File drop naturally — the mmap persists)
    ...
    pub fn publish(&mut self, slot: u64, timestamp_ns: u64, entries_bytes: &[u8]) -> u64 {
        ...
        unsafe {
            ...
            ptr::write_unaligned(entry_ptr.add(16) as *mut u64, timestamp_ns);
            ptr::write_unaligned(entry_ptr.add(24) as *mut u32, data_len as u32);
            ptr::write_unaligned(entry_ptr.add(28) as *mut u32, 0); // padding

            // Entry data (memcpy)
            ptr::copy_nonoverlapping(entries_bytes.as_ptr(), entry_ptr.add(32), data_len);
        }
...
impl ShmemRingConsumer {
    ...
    pub fn poll(&mut self) -> PollResult<'_> {
        ...
        let timestamp_ns = unsafe { ptr::read_unaligned(entry_ptr.add(16) as *const u64) };
        let data_len = unsafe { ptr::read_unaligned(entry_ptr.add(24) as *const u32) } as usize;
...
mod tests {
    ...
    fn test_basic_write_read() {
        ...
        // Write entry
        producer.publish(100, 999, b"test data 123");
    ...
    fn test_multiple_entries() {
        ...
        for i in 0..100u64 {
            ...
            producer.publish(i, i * 1000, data.as_bytes());
        }
    ...
    fn test_wraparound() {
        ...
        let big_data = vec![0xABu8; 512]; // 512 bytes + 32 header = 544 per entry
        // 4096 / 544 ≈ 7.5, so after ~7 entries we wrap

        let mut read_count = 0;
        for i in 0..20u64 {
            producer.publish(i, i, &big_data);
